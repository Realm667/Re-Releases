// Generated from the current two lava shaders.
...
void LipFall(inout Material mat)
{
    ...
    vec3 source=(LipFallTexel(ua).rgb+LipFallTexel(ub).rgb)*0.5;
    // An orange texel of the original LAVA patch; preserves palette changes.
    vec3 tint=LipFallTexel(vec2(0.9453125,0.58203125)).rgb;
    ...
    mat.Base=vec4(color,LipFallTexel(ua).a);
    mat.Normal=normal;
