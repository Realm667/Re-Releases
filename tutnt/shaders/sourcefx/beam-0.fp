...
vec4 ProcessTexel()
{
 // Read raw state data, unaffected by stencil/fog texture modes.
...
#endif
 if(state==1)intensity*=mix(0.65,0.40,nearBoss);
 else if(state>=3)
 {
#if BEAM_KIND == 0
  intensity*=state==3?0.16:0.28;
#else
  intensity*=0.035;
#endif
  if(state>=5)
