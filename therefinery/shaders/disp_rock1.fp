...
{
    return 0.29 - texture(displacement, currentTexCoords).r;
}
...
vec2 ParallaxMap(mat3 tbn)
{
    ...
    const float maxLayers = 22.0;

    // Calculate fragment view direction in tangent space
    mat3 invTBN = transpose(tbn);
    vec3 V = normalize(invTBN * ((uCameraPos.xyz * 0.9999) - (pixelpos.xyz * 0.9996)));
    vec2 T = vTexCoord.st;
    ...
    vec2 currentTexCoords = T + (P * 0.01);
