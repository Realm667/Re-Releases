...
a=p.parse_args();local=ROOT/'tutnt/.codex';out=local/'logs/secondary-fire/coop';out.mkdir(parents=True,exist_ok=True)
reports=local/'validation/secondary-fire';reports.mkdir(parents=True,exist_ok=True)
fixture=local/'builds/utnt-secondary-coop.pk3'
...
with zipfile.ZipFile(fixture,'w',zipfile.ZIP_DEFLATED) as z:
 ...
 for f in ['tests.zc','coop.zc']:z.write(ROOT/'tools/fixtures/secondary-fire'/f,f)
 z.writestr('MAPINFO','gameinfo { AddEventHandlers="SecondaryCoopTest" }\nmap SECCOOP "Secondary coop test" {}\n')
...
try:
 for i in range(2):
  ...
  cfg=out/f'peer-{i}.cfg';cfg.write_text('wait 125; +altattack; wait 1; -altattack\n' if i==0 else '')
  args=[a.engine,'-iwad',a.iwad,'-file',str(a.mod.resolve()),str(fixture),'-config',str(config),'-noautoload','-nosound','-stdout','-noidle','-rngseed','667','+playerclass','Marine','+vid_fullscreen','false','+vid_preferbackend','1','+use_mouse','false','+i_pauseinbackground','false','+map','SECCOOP','+exec',str(cfg)]
...
report={'ok':ok,'peers':results};(reports/'coop.json').write_text(json.dumps(report,indent=2));print(json.dumps(report));raise SystemExit(0 if ok else 1)
