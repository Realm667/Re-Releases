...
def commands(label):
    ...
    for kind,hp,name in [(2,600,'bruisers-60'),(3,300,'guardian-30'),(4,100,'queen-10'),(5,300,'source-shield')]:
        ...
        snap(name)
    result += ['netevent plaque 5 300 0','wait 20']
    snap('source-exposed')
...
def main():
    ...
    for renderer in ['0','1'] if args.renderer=='both' else [args.renderer]:
        ...
        result=run_case(os.environ['UTNT_ENGINE'],os.environ['UTNT_IWAD'],mod=args.mod,
            mapname='TNT01',addon=ROOT/'tools/boss-hud-tests',renderer=renderer,
            label=label,commands=commands(label),timeout=60,regression=True,
            settings=[('screenblocks',11),('con_notifytime',0),('UTNT_bosshud',True),('UTNT_bosspercent',True),('UTNT_reducedfx',False)])
