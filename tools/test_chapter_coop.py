"""Two real peers: independent pages, rejected guest travel and host-authorized map change."""
...
addon=compile_fixture();logs=ROOT/'logs';processes=[];results=[]
si=subprocess.STARTUPINFO();si.dwFlags|=subprocess.STARTF_USESHOWWINDOW;si.wShowWindow=0
try:
 for i in range(2):
  label=f'chapter-coop-{i}';ini=logs/(label+'.ini')
  ini.write_text('[GlobalSettings]\nvid_fullscreen=false\nwin_w=658\nwin_h=527\nvid_maxfps=60\ni_pauseinbackground=false\nvid_activeinbackground=true\n')
  cfg=logs/(label+'.cfg')
  if i==0:commands='wait 330; netevent uitcooppages; wait 40; netevent utnt_chapter 3 1; wait 100; netevent uitmap 2; wait 5; echo UTNT_CHAPTER_COOP_COMPLETE\n'
  else:commands='wait 230; netevent utnt_chapter 0 3; wait 10; netevent utnt_chapter 1 3; wait 10; netevent utnt_chapter 3 3; wait 80; netevent uitcooppages; wait 145; netevent uitmap 2; wait 5; echo UTNT_CHAPTER_COOP_COMPLETE\n'
  ...
  args=[os.environ['UTNT_ENGINE'],'-iwad',os.environ['UTNT_IWAD'],'-file',str(a.mod.resolve()),str(addon),'-config',str(ini),
        ...
        '+language','deu' if i else 'enu','+map','INTERMAP','+exec',cfg.as_posix()]
...
(logs/'chapter-coop-results.json').write_text(json.dumps(results,indent=2)+'\n')
