...
def actor_sources(actors, chunk_size=128):
    """Bound UDB's per-class source rescans without hiding any model classes."""
    if chunk_size < 1:
        raise ValueError('chunk_size must be positive')
    entry = 'tutnt/zscript/sky-edges-generated.zc'
    header = '// Generated by tools/build_sky_edges.py.\n'
    outputs = {}
    includes = []
    for start in range(0, len(actors), chunk_size):
        name = f'zscript/sky-edges-generated/part-{start // chunk_size:03d}.zc'
        outputs['tutnt/' + name] = header + '\n'.join(actors[start:start + chunk_size]) + '\n'
        includes.append(f'#include "{name}"')
    outputs[entry] = header + '\n'.join(includes) + '\n'
    return outputs
...
def generate(root=ROOT,check=False):
    ...
    outputs.update(actor_sources(actors))
    ...
    stale=[]
    for rel in previous:
        if rel not in outputs and re.fullmatch(r'tutnt/(?:models/sky-edges/[\w-]+\.obj|zscript/sky-edges-generated/part-\d+\.zc|skyedges/[\w-]+\.txt|materials/sky-edges/(?:glow-\d+|blend-SG\d+)\.png)',rel):
