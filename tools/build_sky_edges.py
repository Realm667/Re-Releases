...
One-sided sky walls and closed sky sectors qualify. Ordinary outdoor steps,
...
def wall_uv(e, variants, bindings):
    ...
    part=e['part'];suffix='mid' if part==1 else 'bottom'
    ...
    peg=l.get('dontpegbottom','false')=='true'
...
def detect(b, variants, bindings=None):
    ...
    for index,l in enumerate(b['linedef']):
        ...
        for face,key in enumerate(('sidefront','sideback')):
            ...
            part=1 if back is None else 2
            texture=tex(side,'texturemiddle' if part==1 else 'texturebottom')
            ...
            clearance=min(heights[j]-plane(front,p,'floor') for j,p in enumerate((a,z)))
            if clearance<48:continue
            snow=family in SNOW
            radius=min(24 if snow else 22,max(7,clearance*.075))
...
def wave(p, snow):
    ...
    return broad if snow else broad+.13*math.sin(x*.083+y*.047)
...
def profile(r,snow):
    ...
    if not snow:return [(d*r,z*r) for d,z in controls]
...
def mesh(e):
    ...
    snow=e['kind']=='snow';steps=max(2,math.ceil(e['length']/(14 if snow else 22)))
    ...
    out+=['f '+' '.join(f'{v}/{v}/{v}' for v in f) for f in faces]
...
def generate(root=ROOT,check=False):
    ...
    manifest=dict(schema=2,glow_sectors=len(glow_sectors),glow_materials=len(skin_bindings),rule='one-sided sky walls or closed sky sectors; material allowlist',snow=sorted(SNOW),rock=sorted(ROCK),edges=records)
