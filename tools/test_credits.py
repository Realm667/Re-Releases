...
if a.mode=='visual':
 ...
 cmd+=['netevent creditstest 9','netevent creditstest 1 25','wait 15',f'screenshot logs/{label}-chapter.png','save creditremaster','wait 50','netevent creditstest 12','load creditremaster','wait 3','netevent creditstest 13','wait 50','netevent creditstest 12','netevent creditstest 1 26','wait 15',f'screenshot logs/{label}-remaster.png','netevent creditstest 1 27','wait 15',f'screenshot logs/{label}-remaster-next.png','netevent creditstest 7','wait 10','netevent creditstest 8']
elif a.mode=='animation':
 cmd+=['netevent creditstest 10 7 0','wait 8',f'screenshot logs/{label}-entry.png','save creditentry','wait 10','load creditentry','wait 2','netevent creditstest 14','wait 25',f'screenshot logs/{label}-settled.png']
...
r=run_case(ENGINE,IWAD,root=ROOT,mod=a.mod,addon=addon,mapname='ENDMAP01',renderer=a.renderer,label=label,timeout=85,settings=[('win_w',986 if a.classic else 1306),('win_h',791),('screenblocks',12),('con_notifytime',0),('vid_maxfps',60),('i_pauseinbackground',False),('vid_activeinbackground',True),('use_mouse',False),('use_joystick',False),('gl_texture_filter',0)],commands='; '.join(cmd)+'\n')
