...
from material_panel_geometry import panel_height, NAMES as PANEL_NAMES
from material_support_geometry import support_height, NAMES as SUPPORT_NAMES
...
def validate_compatibility(config):
    ...
    for m in materials.values():
        ...
        if detail and not ((detail.get('kind')=='traced-geometry' and detail.get('name') in TRACED_NAMES | SURFACE_NAMES | PANEL_NAMES | SUPPORT_NAMES) or
...
def relief(rgb, profile, depth, logical, detail=None):
    ...
    if detail and detail.get('kind')=='traced-geometry':
        h = PROFILE_BASE[profile]+(support_height if detail['name'] in SUPPORT_NAMES else panel_height if detail['name'] in PANEL_NAMES else surface_height if detail['name'] in SURFACE_NAMES else traced_height)(rgb,logical,detail)/depth
...
def generate(root=ROOT, *, check=False, iwad=None):
    ...
    panel_digest=digest(read(root/'tools/material_panel_geometry.py')+geometry_digest.encode())
    support_digest=digest(read(root/'tools/material_support_geometry.py')+panel_digest.encode())
    ...
    for m in config['materials']:
        ...
        for v in variants:
            ...
            fingerprint=digest(rgb.tobytes()+json.dumps([logical,m['profile'],m['depth'],detail,support_digest if detail and detail.get('name') in SUPPORT_NAMES else panel_digest if detail and detail.get('name') in PANEL_NAMES else surface_digest if detail and detail.get('name') in SURFACE_NAMES else traced_digest if detail and detail.get('kind')=='traced-geometry' else geometry_digest if m['profile']=='authored' else 'signed-127-v1']).encode())[:14]
