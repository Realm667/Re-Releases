...
def capture(renderer='1',live=False,batch=0):
    ...
    label=f'material-final-{"live" if live else "package"}-r{renderer}-b{batch}'
  ...
  int target=-1;'''.replace('CASES',''.join(cases))
    ...
    out=C/'validation/material-final-pass';out.mkdir(exist_ok=True)
    (out/f'{label}.json').write_text(json.dumps(r,indent=2),encoding='utf8')
    print(json.dumps(r),flush=True)

if __name__=='__main__':
    p=argparse.ArgumentParser(description=__doc__);p.add_argument('--renderer',default='1');p.add_argument('--live',action='store_true');p.add_argument('--batch',type=int,default=0);a=p.parse_args();capture(a.renderer,a.live,a.batch)
