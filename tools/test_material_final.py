...
def capture(renderer='1',live=False,batch=0,names=None,suite='final'):
    ...
    assert suite in ('final','next','panel','support')
...
if __name__=='__main__':
    p=argparse.ArgumentParser(description=__doc__);p.add_argument('--renderer',default='1');p.add_argument('--live',action='store_true');p.add_argument('--batch',type=int,default=0);p.add_argument('--names',nargs='+');p.add_argument('--suite',choices=['final','next','panel','support'],default='final');a=p.parse_args();capture(a.renderer,a.live,a.batch,a.names,a.suite)
