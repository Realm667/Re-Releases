...
def run(lang,width,height,renderer='1',mod=None):
    ...
    cmd=['wait 230','event objcheck 1','event objresources',f'screenshot logs/{label}-map1.png',
         '+utnt_objectives', 'wait 2', f'screenshot logs/{label}-map1-manual.png', '-utnt_objectives', 'netevent objremember','wait 3',f'save {label}','wait 5','netevent objshow 2','wait 20',
    ...
    for mapnum,episode in [(2,2),(3,3),(5,4),(6,5),(7,6),(8,7),(9,8),(10,9)]:
        cmd += [f'netevent objshow {mapnum}','wait 18',f'event objcheck {episode}',f'screenshot logs/{label}-episode{episode}-compact.png', f'event objpreview {episode}', 'wait 2', f'screenshot logs/{label}-episode{episode}.png', 'event objpreview 0']
    ...
    if 'Unknown command' in Path(result['log']).read_text(encoding='utf-8'): result['errors'].append('unknown command')
    result['ok']=result['ok'] and not result['errors']
    if not result['ok']: print(Path(result['log']).read_text(encoding='utf-8')[-4500:])
    return result
